 ...
 * Essa clase serve gerenciar os codigos do municipios
 */
abstract class TCodMunicipio
{
     ...
     */
    public static function isCodigoUF($codigo)
    {
        foreach( self::$estados as $chave => $valor ){
            $codigos[] = $valor;
        }
     ...
     * Valida se o codigo passado está presente no registro
     */
    public static function isCodigoMunicipio($codigo)
    {
        foreach( self::$cidades as $chave => $valor ){
            $codigos[] = $chave;
        }
        if( in_array($codigo, $codigos) ){
            return true;
    ...
    /**
     * Busca o dados do muncipio pelo codigo
     */
    public static function buscaMunicipio($codigo)
    {
        $codUf = substr($codigo,0,2);
        foreach( self::$estados as $chave => $valor ){
            if ( $chave == $codUf ) {
                $retorno['uf'] = $valor;
                break;
        ...
        foreach( self::$cidades as $chave => $valor ){
            if ( $chave == $codigo ) {
                $retorno['cidade'] = $valor;
                break;
            }
        }

        if ( empty($retorno['uf']) && empty($retorno['cidade']) ) {
            throw new \InvalidArgumentException("O código '{$codigo}' não encontrou nenhuma cidade");
        }
        return $retorno;
    }
}